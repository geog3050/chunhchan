...
def import_data(filename):
    ...
    participants = []
    with open(filename) as pokemonss:
        pokemon_data = csv.reader(pokemonss, delimiter=',')
...
#output: [['Baltoy', 'Ground', 325.0, 36.0], ['Flareon', 'Fire', 700.0, 11.0], ['Alomomola', 'Water', 165.0, 76.0], ['Gogoat', 'Grass', 367.0, 32.0], ['Pikachu', 'Electric', 243.0, 54.0], ['Luxio', 'Electric', 124.0, 105.0]]
...
def fight(participant1, participant2, first2attack):
    ...
    while (health1 > 0) and (health2 > 0):
        if (first2attack == 1):
            health2 -= attack_multiplier(type1, type2) * attack1
        elif (first2attack == 2):
            health1 -= attack_multiplier(type2, type1) * attack2
        rounds += 1
        first2attack = 1 if first2attack == 2 else 2

    winner = 1 if health1 > 0 else 2
...
def tournament(participants):
    ...
    wins = [0 for i in range(0, len(participants))]
    for i in range(0, len(participants)):
        for j in range(i + 1, len(participants)):
            home_results = fight(participants[i], participants[j], 1)
            away_results = fight(participants[i], participants[j], 2)
            if (home_results[0] == 1):
                wins[i] += 1
            else:
                wins[j] += 1
