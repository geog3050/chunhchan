...
def importCSVIntoGeodatabase(csvFile, geodatabase):
    return arcpy.conversion.TableToGeodatabase(csvFile, geodatabase)
...
def krigingFromPointCSV(inTable, valueField, xField, yField, inClipFc, workspace = "assignment3.gdb"):
    try:
        ...
        kriging_layer = arcpy.management.MakeXYEventLayer(inTable, xField, yField, "kriging_layer", arcpy.SpatialReference(4326))
        arcpy.management.Project(kriging_layer, "projected_kriging_layer", inClipFc.spatialReference)
        arcpy.CheckOutExtension("Spatial")
        kriging_result = arcpy.sa.Kriging("projected_kriging_layer", valueField)
        arcpy.management.Clip(kriging_result, inClipFc, clipped_raster)
